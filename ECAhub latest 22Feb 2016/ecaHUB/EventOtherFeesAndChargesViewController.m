...
@interface EventOtherFeesAndChargesViewController () {
    ...
    NSInteger isselectCurrency;
    
}

@end

@implementation EventOtherFeesAndChargesViewController
...
- (void)viewDidLoad {
    ...
    // bookMaterialBtn.hidden = YES;
    
    //securityBtn.hidden = YES;
    
    // bookMaterialText.hidden = YES;
    
    // securityDepositText.hidden = YES;
    
    //currencyBtn.hidden = YES;
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        ...
        [scroll_view setContentSize:CGSizeMake(self.view.frame.size.width, 950)];
        
    } else {
        
        storyboard = [UIStoryboard storyboardWithName:@"Main_Iphone" bundle:nil];
        
        [scroll_view setContentSize:CGSizeMake(self.view.frame.size.width, 450)];
    }
        ...
        [setConn startConnectionToUploadMultipleImagesWithString:[NSString stringWithFormat:@"add_events"] images:all_imageArray HttpMethodType:Post_Type HttpBodyType:paramURL Output:^(NSDictionary *receivedData){
            ...
            if ([setConn responseCode] == 1) {
                ...
                if ([[receivedData valueForKey:@"code"] integerValue] == 1) {
                    ...
                    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:Alert_title message:@"You have just completed Step 1 by saving the Event Description and Terms & Conditions. You are nearly finished. Now complete Step 2 by scheduling Session Options. You need at least one Session Option for members to enroll in your Event." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
...
- (IBAction)tap_infofeesBtn:(id)sender {
